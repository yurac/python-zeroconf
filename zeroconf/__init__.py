...
def service_type_name(type_, strict=True):
    """
...
class QuietLogger:
    ...
    @classmethod
    def log_exception_warning(cls, *logger_data):
    ...
    def log_warning_once(cls, *args):
        ...
        if msg_str not in cls._seen_logs:
            ...
            logger = log.debug
        cls._seen_logs[msg_str] = cls._seen_logs[msg_str] + 1
        logger(*args)
...
class DNSEntry:
    ...
    def entry_to_string(self, hdr, other):
...
class DNSRecord(DNSEntry):
    ...
    def to_string(self, other):
        """String representation with additional information"""
        arg = "%s/%s,%s" % (self.ttl, int(self.get_remaining_ttl(current_time_millis())), other)
...
class DNSIncoming(QuietLogger):
    ...
    def read_utf(self, offset, length):
        ...
        return str(self.data[offset : offset + length]).decode('utf-8', 'replace')
...
class DNSOutgoing:
    ...
    def reset_for_next_packet(self):
        self.names = {}
        self.data = []
        self.size = 12
    ...
    def write_record(self, record, now, allow_long=False):
...
class ServiceBrowser(RecordUpdateListener):
    ...
    def __init__(self, zc, type_, handlers=None, listener=None, addr=None, port=_MDNS_PORT, delay=_BROWSER_TIME):
        ...
        self._handlers_to_call = OrderedDict()
    ...
    def update_record(self, zc, now, record):
        ...
        def enqueue_callback(state_change, type_, name):
...
class ServiceInfo(RecordUpdateListener):
    ...
    * addresses and parsed_addresses: List of IP addresses (either as bytes, network byte order, or in parsed
    ...
    # fmt: off
    def __init__(self, type_, name, port=None, weight=0, priority=0, properties=b'', server=None, host_ttl=_DNS_HOST_TTL, other_ttl=_DNS_OTHER_TTL, \
        addresses=None, parsed_addresses=None):
    ...
    def parsed_addresses(self, version=IPVersion.All):
...
def ip6_to_address_and_index(adapters, ip):
    ipaddr = ip_address_from_str(ip)
    for adapter in adapters:
        for adapter_ip in adapter.ips:
            # IPv6 addresses are represented as tuples
            if isinstance(adapter_ip.ip, tuple) and ip_address_from_str(adapter_ip.ip[0]) == ipaddr:
                return (adapter_ip.ip, adapter.index)

    raise RuntimeError('No adapter found for IP address %s' % ip)

def interface_index_to_ip6_address(adapters, index):
    for adapter in adapters:
...
def ip6_addresses_to_indexes(interfaces):
    ...
    for iface in interfaces:
        ...
        elif isinstance(iface, str) and ip_address_from_str(iface).version == 6:
            result.append(ip6_to_address_and_index(adapters, iface))

    return result

def normalize_interface_choice(choice, ip_version=IPVersion.V4Only):
    ...
    if choice is InterfaceChoice.Default:
        ...
        result = [i for i in choice if isinstance(i, str) and ip_address_from_str(i).version == 4]
...
def new_socket(bind_addr, port=_MDNS_PORT, ip_version=IPVersion.V4Only, apple_p2p=False):
    log.debug(
    ...
    bind_addr = tuple([bind_addr[0]] + [port] + list(bind_addr[1:]))
    s.bind(bind_addr)
...
def add_multicast_member(listen_socket, interface):
    ...
    try:
        if is_v6:
            iface_bin = struct.pack('@I', interface[1])
...
def new_respond_socket(interface, apple_p2p = False):
    is_v6 = isinstance(interface, tuple)
...
def create_sockets(interfaces=InterfaceChoice.All, unicast=False, ip_version=IPVersion.V4Only, apple_p2p=False):
    if unicast:
        listen_socket = None
    else:
    ...
    if not unicast and interfaces is InterfaceChoice.Default:
        for i in normalized_interfaces:
            add_multicast_member(listen_socket, i)
...
def can_send_to(sock, address):
    if isinstance(address, str):
        addr = ip_address_from_str(address)
    else:
...
class Zeroconf(QuietLogger):
    ...
    def __init__(self, interfaces=InterfaceChoice.All, unicast=False, ip_version=None, apple_p2p=False):
        ...
        if ip_version is None and isinstance(interfaces, list):
            has_v6 = any(
                isinstance(i, int) or (isinstance(i, str) and ip_address_from_str(i).version == 6)
                for i in interfaces
            )
            has_v4 = any(isinstance(i, str) and ip_address_from_str(i).version == 4 for i in interfaces)
    ...
    def register_service(self, info, ttl=None, allow_name_change=False, cooperating_responders=False):
    ...
    def _add_broadcast_answer(self, out, info, override_ttl):
    ...
    def handle_response(self, msg):
        ...
        for record in msg.answers:
            ...
            if record.unique:  # https://tools.ietf.org/html/rfc6762#section-10.2
                ...
                for entry in list(self.cache.entries_with_name(record.name)):
    ...
    def handle_query(self, msg, addr, port):
        ...
        for question in msg.questions:
            if question.type == _TYPE_PTR:
                ...
                service = self.registry.get_info_name(name_to_find)
                if service is None:
                    continue
        ...
        for packet in packets:
            ...
            for s in self._respond_sockets:
                ...
                try:
                    ...
                    log.warning('Error sending through socket', exc_info=True)
                else:
